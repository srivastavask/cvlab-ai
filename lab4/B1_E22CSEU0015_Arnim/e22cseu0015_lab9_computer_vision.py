# -*- coding: utf-8 -*-
"""E22CSEU0015_LAB9_Computer Vision.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uuJxFMFcHeDpVaP2nu3lYfDBzxIdhy5h
"""
...
damaged_img = cv2.imread(filename=r"d1.png")
...
def stitch_images(img1_path, img2_path, output_path=None):
    """
    ...
    output_path (str, optional): Path to save the output panorama

    Returns:
    numpy.ndarray: The stitched panorama image
    """
    # Read the images
    ...
    if len(good_matches) > 10:
        # Extract source and destination points for homography
        ...
        # Create points for perspective transform
        pts1 = np.float32([[0, 0], [0, h1], [w1, h1], [w1, 0]]).reshape(-1, 1, 2)

        # Apply perspective transform
        pts2 = cv2.perspectiveTransform(pts1, H)

        # Find min and max points to establish size of the result image
        pts = np.concatenate((pts2, np.float32([[0, 0], [0, h2], [w2, h2], [w2, 0]]).reshape(-1, 1, 2)), axis=0)

        [xmin, ymin] = np.int32(pts.min(axis=0).ravel() - 0.5)
        [xmax, ymax] = np.int32(pts.max(axis=0).ravel() + 0.5)

        # Translation matrix to move the image to positive coordinates if needed
        t = [-xmin, -ymin]
        ...
        # Convert result to RGB for display/saving
        result_rgb = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)

        # Save the panorama if output path is provided
        if output_path:
...
if __name__ == "__main__":
    ...
    # Stitch images
    result = stitch_images(img1_path, img2_path, output_path)
...
!unzip -q dogs-breed-dataset.zip -d /content/dogs_dataset

import os

for root, dirs, files in os.walk("/content/dogs_dataset"):
    level = root.replace("/content/dogs_dataset", "").count(os.sep)
    indent = " " * 4 * level
...
# ✅ Change this based on folder structure
DATASET_PATH = '/content/dogs_dataset'
